...
BOOL 
Init(){
	iFrameBuffer = open ("/dev/fb0", O_RDWR);
	if(!iFrameBuffer){
		printerror("open framebuffer return error.");
		return FALSE;
	}
	ioctl (iFrameBuffer, FBIOGET_VSCREENINFO, &vInfo);
	...
	vInfo.xoffset = 0;
	vInfo.yoffset = 0;
	ioctl (iFrameBuffer, FBIOPAN_DISPLAY, &vInfo);
	pFrameBuffer =mmap (NULL, iBufferSize, PROT_READ | PROT_WRITE, MAP_SHARED, iFrameBuffer, 0);
	if(!pFrameBuffer){
	...
	X86EmuGalDev.ScreenHeight=iFrameHeight;
	X86EmuGalDev.ScreenWidth=iFrameWidth;
	X86EmuGalDev.iBytesDataType=iBytesDataType;
	return true;
}
